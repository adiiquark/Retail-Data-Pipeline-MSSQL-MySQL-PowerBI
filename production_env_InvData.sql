...
exec sp_rename '[dbo].[inventory_production_with_issues].InvID', 'OrderID', 'COLUMN';
...
-- 2570 bad availability, 2542 bad demand
...
update [dbo].[inventory_production_with_issues]
set Demand = 5000
where Demand > 5000;
...
Delete from [dbo].[inventory_production_with_issues]
...
select OrderDate from [dbo].[inventory_production_with_issues]
where OrderDate < '2022-01-01' OR OrderDate >GETDATE();
...
pdts.unitPrice
from 
[dbo].[inventory_production_with_issues] as Inv 
left join
...
select count(*) FROM [dbo].[inventory_production_with_issues];
...
--view the table
select * from [dbo].[ProductOrderAvailability]
-------
SELECT TOP 20 *
FROM dbo.ProductOrderAvailability
ORDER BY NEWID(); 