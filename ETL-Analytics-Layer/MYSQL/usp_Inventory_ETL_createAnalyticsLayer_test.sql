...
CREATE DATABASE IF NOT EXISTS test_inv;
USE test_inv;
...
-- Count number of records in inventory_test_with_issues
SELECT COUNT(*) AS Total_Records FROM inventory_test_with_issues;
--  5100 records

-- View the data 
SELECT * FROM products_table LIMIT 10;
SELECT * FROM inventory_test_with_issues LIMIT 10;
...
UPDATE inventory_test_with_issues i
JOIN (
    SELECT
        ProductID,
...
-- 4.2. DEDUPLICATION
CREATE TABLE inventory_test_with_issues_deduplicated AS
WITH NumberedRecords AS (
    SELECT
        *,
        ROW_NUMBER() OVER(PARTITION BY ProductID, WarehouseLocation, SnapshotDate ORDER BY SnapshotInvID) AS rn
...
FROM
    inventory_test_with_issues_deduplicated
GROUP BY
    ProductID,
    WarehouseLocation,
    SnapshotDate
...
SET @DemandCap = (SELECT PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY unitsDemanded) FROM inventory_test_with_issues_deduplicated);
SET @AvailabilityCap = (SELECT PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY unitsAvailable) FROM inventory_test_with_issues_deduplicated);
...
UPDATE inventory_test_with_issues_deduplicated
...
SELECT
    ...
    COUNT(*) AS IssueCount
FROM inventory_test_with_issues_deduplicated
...
CREATE TABLE test_inv_Clean AS
SELECT
    SnapshotInvID,
    ProductID,
    ProductName,
...
SELECT 'Business logic applied. New table test_inv_Clean created.' AS 'Status';
...
SELECT DISTINCT WarehouseLocation FROM test_inv_Clean;
