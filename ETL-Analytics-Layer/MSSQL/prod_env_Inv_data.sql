...
select count(*) as Total_Records from [dbo].[inventory_production_with_issues];
--51000 records
...
select * from [dbo].[inventory_production_with_issues];
...
from [dbo].[inventory_production_with_issues];
...
from [dbo].[inventory_production_with_issues] i
join (
    select ProductID,
...
with NumberedRecords as (
    ...
    from
        [dbo].[inventory_production_with_issues] 
)
...
select 
    ...
    SnapshotDate
into 
    [dbo].[inventory_production_with_issues_deduplicated] -- This will be our new, clean table
from 
    NumberedRecords
where 
...
FROM 
    [dbo].[inventory_production_with_issues_deduplicated]
GROUP BY 
    ProductID, 
    WarehouseLocation, 
    SnapshotDate
...
from [dbo].[inventory_production_with_issues_deduplicated];
...
update [dbo].[inventory_production_with_issues_deduplicated]
set 
...
into 
    DimWarehouse
from 
    (SELECT DISTINCT WarehouseLocation from [dbo].[production_inv_Clean]) as UniqueWarehouses;
...
into 
    FactInventory
from 
    [dbo].[production_inv_Clean] c -- Using our cleaned source table
INNER JOIN 
